...
This repository uses a mostly flat Python layout. Core game logic lives in `connect_four.py`, including `Game`, `Board`, `Player`, and enums. The bot AI and minimax evaluation logic live in `bot_engine.py`, while `game_loop.py` is the CLI entry point. Keep core game tests in `test.py` and BotEngine-specific coverage in `test_bot_engine.py`. Benchmark scripts and saved outputs live under `benchmarks/`, and scripted first-player regression sequences are documented in `perfect_plays.txt`.
...
- `python3 test.py` executes the current assertion-based regression checks.
- `python3 -m pytest test_bot_engine.py` runs BotEngine regression and heuristic-consistency tests.
- `python3 benchmarks/benchmark_bot.py` measures bot performance and search strength.
- `python3 benchmarks/benchmark_bot.py --depth-max 8 --games 20` runs a heavier benchmark sweep.
- `python3 benchmarks/depth_move_compare.py` compares time-per-move across depths `3-8` on shared first-20 positions.
...
## Testing Guidelines
Add rule, turn-order, draw, and win checks to `test.py`. Put bot-search, move-choice, incremental-score, undo-restore, and scripted perfect-play regressions in `test_bot_engine.py`. Prefer parametrized pytest cases for repeated board setups. For search or heuristic changes, verify `BotEngine.choseMove()` still matches `_choose_move_reference()` on fixed boards and re-run the `perfect_plays.txt` sequences across depths `1, 2, 3, 5, 8`.
